...
export function createSearchEmailsTool(grantId: string, onEvent?: (e: any) => void) {
  return tool({
    ...
    parameters: z.object({
      ...
      topK: z.number().nullable().describe('Number of results to return'),
      dateFrom: z.string().nullable().describe('Filter emails from this date (ISO 8601)'),
      dateTo: z.string().nullable().describe('Filter emails to this date (ISO 8601)'),
    }),
    async execute({ query, topK, dateFrom, dateTo }) {
      ...
      const results = await hybridQuery({
        ...
        vector: vec,
        sparseEmbedding,
        topK: (typeof topK === 'number' && isFinite(topK) ? Math.max(1, Math.min(200, topK)) : 10),
        filter,
      });
...
export function createListUnreadTool(grantId: string, onEvent?: (e: any) => void) {
  return tool({
    ...
    parameters: z.object({
      limit: z.number().nullable().describe('Maximum number of unread messages to return'),
    }),
    async execute({ limit }) {
      const _t0 = Date.now();
      ...
      url.searchParams.set('limit', String(typeof limit === 'number' && isFinite(limit) ? Math.max(1, Math.min(100, limit)) : 10));
...
export function createListContactsTool(grantId: string, onEvent?: (e: any) => void) {
  return tool({
    ...
    parameters: z.object({
      limit: z.number().nullable().describe('Maximum number of contacts to return'),
    }),
    async execute({ limit }) {
      const _t0 = Date.now();
...
export function createListEventsTool(grantId: string, onEvent?: (e: any) => void) {
  return tool({
    ...
    parameters: z.object({
      limit: z.number().nullable().describe('Maximum number of events to return'),
      calendarId: z.string().nullable().describe('Calendar ID to query'),
    }),
    async execute({ limit, calendarId }) {
      ...
      url.searchParams.set('limit', String(typeof limit === 'number' && isFinite(limit) ? Math.max(1, Math.min(100, limit)) : 10));
      url.searchParams.set('calendar_id', String(calendarId ?? 'primary'));
...
export function createTriageRecentEmailsTool(grantId: string, onEvent?: (e: any) => void) {
  return tool({
    ...
    parameters: z.object({
      limit: z.number().nullable().describe('Number of recent emails to triage'),
    }),
    async execute({ limit }) {
      const _t0 = Date.now();
      ...
      const results = await hybridQuery({
        ...
        vector: vec,
        sparseEmbedding,
        topK: (typeof limit === 'number' && isFinite(limit) ? Math.max(1, Math.min(200, limit)) : 50),
...
export function createAgentTools(grantId: string, onEvent?: (e: any) => void) {
  ...
  return {
    email: [
      createSearchEmailsTool(grantId, withAgent('email_ops')),
      createListUnreadTool(grantId, withAgent('email_ops')),
      createTriageRecentEmailsTool(grantId, withAgent('email_ops')),
    ],
    insights: [
      createAggregateEmailsTool(grantId, withAgent('insights')),
      createSearchEmailsTool(grantId, withAgent('insights')), // Insights can also search
    ],
    contacts: [
      createListContactsTool(grantId, withAgent('contacts')),
    ],
    calendar: [
      createListEventsTool(grantId, withAgent('calendar')),
    ],
  };
}
