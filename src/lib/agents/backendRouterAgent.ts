...
export function createBackendRouterAgent(
  ...
  const agent = new Agent({
    ...
    instructions: () => {
      ...
      return [
        'You are the RouterAgent orchestrating this conversation.',
        'Available specialists and their focus areas:',
        capabilityLines,
        'Your job: reason about the conversation and decide which specialist should handle the task.',
        'When confident, delegate to the appropriate specialist using handoffs.',
        'After the specialist completes, present a concise summary that cites the actual tool outputs by name.',
      ].join('\n');
    },
